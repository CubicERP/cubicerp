...
def configure_parser():
    ...
    group = optparse.OptionGroup(parser, "Note",
        ...
        "with the CubicERP server for case management in the CRM module.")
    parser.add_option_group(group)
    parser.add_option("-u", "--user", dest="userid",
                      help="CubicERP user id to connect with",
                      default=config.OPENERP_DEFAULT_USER_ID, type='int')
    parser.add_option("-p", "--password", dest="password",
                      help="CubicERP user password",
    ...
    parser.add_option("-d", "--dbname", dest="dbname",
                      help="CubicERP database name (default: %default)",
                      default=config.OPENERP_DEFAULT_DATABASE)
    parser.add_option("--host", dest="host",
                      help="CubicERP Server hostname",
                      default=config.OPENERP_HOSTNAME)
    parser.add_option("--port", dest="port",
                      help="CubicERP Server XML-RPC port number",
...
def main():
    """
    Receive the email via the stdin and send it to the CubicERP Server
    """
    ...
    try:
        ...
        subject = '[CubicERP]:ERROR: Mailgateway - %s' % time.strftime('%Y-%m-%d %H:%M:%S')
        send_mail(
            config.MAIL_ERROR,
            config.MAIL_ADMINS,
            subject, msg, files=[('message.txt', msg_txt)]
        )
        sys.stderr.write("Failed to deliver email to CubicERP Server, sending error notification to %s\n" % config.MAIL_ADMINS)
