...
class AccountChartTemplate(models.Model):
    ...
    def generate_journals(self, acc_template_ref, company, journals_dict=None):
        journal_to_add = [{'name': _('Stock Journal'), 'type': 'stock', 'code': 'STJ', 'favorite': False, 'sequence': 8}]
    ...
    def generate_properties(self, acc_template_ref, company, property_list=None):
        ...
        value = self.env['account.journal'].search([('company_id', '=', company.id), ('code', '=', 'STJ'), ('type', '=', 'stock')], limit=1)
        if value:
